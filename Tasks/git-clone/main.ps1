...
if (!(Get-Command git -ErrorAction SilentlyContinue)) {
    ...
    if (!(Get-Command git -ErrorAction SilentlyContinue)) {
        ...
        $process = Start-Process -FilePath "C:\Program Files\PowerShell\7\pwsh.exe" -ArgumentList "-MTA -Command `"Install-WinGetPackage -Id Git.Git -Source winget | ConvertTo-Json -Depth 10 > $($tempOutFile)`"" -PassThru
